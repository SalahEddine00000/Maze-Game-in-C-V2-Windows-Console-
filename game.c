...
{
    Game_Mode_easy = 0,    // full timer_duration
    Game_Mode_normal = 20, // -%15 timer_duration
    Game_Mode_hard = 50,   // -%50 timer_duration
} Game_Mode_t;
...
Player player1;
int game_loop = 1;
int game_mode = Game_Mode_hard;
unsigned int timer_duration = 60 * 3; // 3 min
...
void hideCursor()
{
    HANDLE consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE); // Get handle to the console output
    CONSOLE_CURSOR_INFO cursorInfo;                         // Structure to hold cursor information

    GetConsoleCursorInfo(consoleHandle, &cursorInfo); // Retrieve current cursor information
    cursorInfo.bVisible = FALSE;                      // Set the cursor to be invisible
    SetConsoleCursorInfo(consoleHandle, &cursorInfo); // Apply the new cursor information
}
...
void init_Maze()
{
    ...
    {
        memset(Maze[y], Wall, MAX_W * sizeof(char));
...
void print_Maze()
{
    ...
    // just for MAX_H 21
    //  XD
    //  printf("%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n", Maze[0], Maze[1], Maze[2], Maze[3], Maze[4], Maze[5], Maze[6], Maze[7], Maze[8], Maze[9], Maze[10], Maze[11], Maze[12], Maze[13], Maze[14], Maze[15], Maze[16], Maze[17], Maze[18], Maze[19], Maze[20]);
}
...
void load_level()
{
    timer = timer_duration - timer_duration * game_mode / 100.0f;
    aldous_broder(); // Generate Maze
    Maze[MAX_H - 2][MAX_W - 3] = Exite;
}
void end()
{
    printf("END (^-^)\n");
...
void move_player()
{
    ...
    if (Maze[player1.pos.y][player1.pos.x] == Exite)
    {
        player1.Level++;
        player1.pos.x = player1.pos.y = 1;
        load_level();
...
int main()
{
    ...
    player1.Level = 1;
    hideCursor();
    load_level(); // generate lev and set timer to 5 min
    DWORD lastTime = GetTickCount();
    // Play the music in loop, asynchronously
    if (!PlaySound(TEXT("background.wav"), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP))
    {
        printf("Failed to play sound. Check file path and format.\n");
        return 1;
    }
    while (game_loop)
    {
        ...
        if (Maze[player1.pos.y][player1.pos.x] != Wall)
        ...
        {
            Maze[collectible.y][collectible.x] = Exite;
        }

        printf("LV : %d\n", player1.Level);
        print_Maze();

        printf("timer  %0.2d:%0.2d\n", timer / 60, timer % 60);
